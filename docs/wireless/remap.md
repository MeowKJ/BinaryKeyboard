...
::: tip 型号使用方式
无线 5KEY / KNOB 的默认键位、FN 行为、蓝牙配对和旋钮说明见 [使用指南](../usage.md#无线版-5key)。
:::

### 1. 切换到 USB 模式

1. 用 USB-C 连接键盘到电脑
2. 如果键盘仍在 BLE 模式，短按 `FN1` 切换到 USB 模式
3. 设备会重启进入 USB 模式，此时蓝牙连接会断开
...
改键配置会保存在键盘内部，但写入动作必须通过 USB 完成。请用 USB 连接 Studio，保存成功后再切回 BLE 模式使用。

### 如何重新配对？

切到 BLE 模式后长按 `FN2` 清除配对信息；同时在电脑 / 手机蓝牙列表中删除旧设备，再重新搜索 `BinaryKeyboard5KEY` 或 `BinaryKeyboardKNOB`。

### 如何恢复默认配置？

优先在 Studio 中执行恢复出厂设置。若设备无法进入 Studio，使用 `CH592F-<MODEL>-<version>-full.hex` 重新首刷 / 恢复。
