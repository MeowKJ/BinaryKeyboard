<template>
  <div class="app-container">
    ...
    <div class="flex flex-column justify-center gap-6" v-if="deviceStore.device">
      ...
      <KnobKeyboard v-if="deviceStore.deviceModelNumber === KeyboardType.KNOB"
        v-model:key-config-list="deviceStore.keyMappingsList" />
      <BasicKeyboard v-if="deviceStore.deviceModelNumber === KeyboardType.BASIC"
        v-model:key-config-list="deviceStore.keyMappingsList" />
      <FiveKeysKeyboard v-if="deviceStore.deviceModelNumber === KeyboardType.FIVE_KEYS"
        v-model:key-config-list="deviceStore.keyMappingsList" />
...
import { CMD_KEYBOARD_SEND, KEYBOARD_FILTER, REPORT_ID_RECEIVE, REPORT_ID_SEND } from '@/utils/deviceConstants';
...
import BasicKeyboard from './components/keyboards/BasicKeyboard.vue';
import FiveKeysKeyboard from './components/keyboards/FiveKeysKeyboard.vue';
...
const updateDeviceData = (dataView: DataView) => {
  ...
  console.log('固件版本:', deviceStore.deviceFirmwareVersion);
  console.log('设备型号:', deviceStore.deviceModelNumber);
...
const onGetReport = (event: HIDInputReportEvent) => {
  if (event.reportId === REPORT_ID_RECEIVE) {
    ...
    showToast('success', '接收数据', '已接收设备数据');
  }
};
