...
```bash
MEOWISP_VERSION=$(python3 tools/scripts/versioning.py show --component meowisp | python3 -c "import json,sys; print(json.load(sys.stdin)['version'])")
python3 tools/meowisp/scripts/package_portable.py \
  --platform macos \
  --arch arm64 \
  --version "$MEOWISP_VERSION" \
