...
impl<'a> Flashing<'a> {
    ...
    pub fn flash(&mut self, raw: &[u8]) -> Result<()> {
        ...
        let result = self.flash_with_callback(raw, |written, _total| {
    ...
    pub fn flash_with_callback<F>(&mut self, raw: &[u8], mut callback: F) -> Result<()>
    where
        F: FnMut(usize, usize),
    {
        ...
        for ch in raw.chunks(CHUNK) {
            ...
            callback(address as usize, total);
        }
    ...
    pub fn verify(&mut self, raw: &[u8]) -> Result<()> {
        let bar = ProgressBar::new(raw.len() as _);
        let result = self.verify_with_callback(raw, |written, _total| {
            bar.set_position(written as u64);
        });
        bar.finish();
        result
    ...
    pub fn verify_with_callback<F>(&mut self, raw: &[u8], mut callback: F) -> Result<()>
    where
        F: FnMut(usize, usize),
    {
        let key = self.xor_key();
